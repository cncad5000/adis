...
ANDT_PID_FILE = INSTALL_DIR / "sbadispid.log"
LIST_FILE = INSTALL_DIR / "list.txt"
LOG_FILE = INSTALL_DIR / "adis.log"
DEBUG_LOG = INSTALL_DIR / "python_debug.log"
CUSTOM_DOMAIN_FILE = INSTALL_DIR / "custom_domain.txt" # 存储最终使用的域名

# ====== 全局可配置参数（可直接在此处修改） ======
USER_NAME = "openand"         # 用户名
UUID = "febaa61d-15b7-4dda-a8f3-c40c49b58f6f"                     # UUID，留空则自动生成
...
# 添加命令行参数解析
def parse_args():
    parser = argparse.ArgumentParser(description="AndtIS Python3 一键脚本 (支持自定义域名和Andt Token)")
    ...
    parser.add_argument("--agk", "--token", dest="agk", help="设置 Andt Tunnel Token (用于Cloudflare Zero Trust命名隧道)")
...
def print_info():
    ...
    print("\033[36m│             \033[33m✨ AndtIS Python3 自定义域名版 ✨              \033[36m│\033[0m")
    print("\033[36m├───────────────────────────────────────────────────────────────┤\033[0m")
    ...
    print("\033[36m│ \033[32m版本: 25.7.0 (支持Andt Token及交互式输入)                 \033[36m│\033[0m")
    print("\033[36m╰───────────────────────────────────────────────────────────────╯\033[0m")
...
def print_usage():
    ...
    print("  \033[36mpython3 script.py --agk YOUR_TOKEN\033[0m     - 使用Andt Tunnel Token安装")
    ...
    print("  \033[36mexport agk=YOUR_ANDT_TUNNEL_TOKEN\033[0m       - 设置Andt Tunnel Token")
    print()
...
def generate_links(domain, port_vm_ws, uuid_str):
    ...
    list_content_color_file.append("\033[36m│                \033[33m✨ AndtIS 节点信息 ✨                   \033[36m│\033[0m")
    list_content_color_file.append("\033[36m├───────────────────────────────────────────────────────────────┤\033[0m")
...
def install(args):
    ...
    # Andt Tunnel Token (agk)
    adis_token = args.agk or os.environ.get("agk") or CF_TOKEN
    if not adis_token:
        adis_token_input = input("请输入 Andt Tunnel Token (AGK) (例如: eyJhIjo...Ifs9, 若使用Cloudflare Zero Trust隧道请输入, 留空则使用临时隧道): ").strip()
        adis_token = adis_token_input or None # None if empty
    if adis_token:
        print(f"使用 Andt Tunnel Token: ******{adis_token[-6:]}") # 仅显示末尾几位
        write_debug_log(f"Andt Token: Present (not logged for security)")
    else:
        print("未提供 Andt Tunnel Token，将使用临时隧道 (Quick Tunnel)。")
        write_debug_log("Andt Token: Not provided, using Quick Tunnel.")
    # Custom Domain (agn)
    custom_domain = args.agn or os.environ.get("agn") or DOMAIN
    if not custom_domain:
        domain_prompt = "请输入自定义域名 (例如: test.zmkk.fun"
        if adis_token:
            domain_prompt += ", 必须是与Andt Token关联的域名"
        else:
    ...
    elif adis_token: # 如果用了token，必须提供域名
        print("\033[31m错误: 使用 Andt Tunnel Token 时必须提供自定义域名 (agn/--domain)。\033[0m")
        sys.exit(1)
    else:
    ...
    config_data = {
        ...
        "adis_token": adis_token, # Will be None if not provided
    ...
    if not adis_token and not custom_domain: # Quick tunnel and no pre-set domain
        print("正在等待临时隧道域名生成...")
        final_domain = get_tunnel_domain()
        if not final_domain:
            print("\033[31m无法获取tunnel域名。请检查adis.log或尝试手动指定域名。\033[0m")
            ...
            sys.exit(1)
    elif adis_token and not custom_domain: # Should have exited earlier, but as a safeguard
        print("\033[31m错误: 使用Andt Token时，自定义域名是必需的但未提供。\033[0m")
        sys.exit(1)
    if final_domain:
        # 生成所有节点链接
        all_links = []
...
def uninstall():
    ...
    # 停止服务
    for pid_file_path in [SB_PID_FILE, ANDT_PID_FILE]:
...
# 升级脚本
def upgrade():
    script_url = "https://raw.githubusercontent.com/yonggekkk/AndtIS/main/agsb_custom_domain.py" # 假设这是最新脚本的地址
...
def check_status():
    ...
    cf_running = ANDT_PID_FILE.exists() and os.path.exists(f"/proc/{ANDT_PID_FILE.read_text().strip()}")
    ...
    if sb_running and cf_running and LIST_FILE.exists():
        ...
        if CUSTOM_DOMAIN_FILE.exists():
            ...
            elif not config.get("adis_token") and LOG_FILE.exists(): # Quick tunnel, try log
                log_content = LOG_FILE.read_text()
                match = re.search(r'https://([a-zA-Z0-9.-]+\.trycloudflare\.com)', log_content)
                if match:
                    domain_to_display = match.group(1)
                    print(f"\033[36m│ \033[32mAndt临时域名: \033[0m{domain_to_display}")
    ...
    print("\033[36m├───────────────────────────────────────────────────────────────┤\033[0m")
    if status_msgs:
        print("\033[36m│ \033[31mAndtIS 服务异常:\033[0m")
        ...
        print("\033[36m│ \033[32m尝试重新安装或检查日志: \033[33mpython3 " + os.path.basename(__file__) + " install\033[0m")
         ...
         print("\033[36m│ \033[31mAndtIS 未运行或配置不完整。\033[0m")
...
def create_startup_script():
    ...
    adis_token = config.get("adis_token") # Safely get token, might be None
...
'''
    ...
    if adis_token: # 使用命名隧道
        cf_cmd = f"{cf_cmd_base} run --token {adis_token}"
    else: # 使用临时隧道
...
def main():
    ...
    if args.action == "install":
        ...
        if INSTALL_DIR.exists() and CONFIG_FILE.exists() and SB_PID_FILE.exists() and ANDT_PID_FILE.exists():
            print("\033[33m检测到AndtIS可能已安装并正在运行。\033[0m")
            if check_status():
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 1: # 如果只运行脚本名，没有其他参数
        ...
        if INSTALL_DIR.exists() and CONFIG_FILE.exists() and SB_PID_FILE.exists() and ANDT_PID_FILE.exists():
            print(f"\033[33m检测到 AndtIS 可能已安装。显示当前状态。\033[0m")
