...
def install(args):
    ...
    andt_token = args.agk or os.environ.get("agk")
    if not andt_token:
        andt_token_input = input("请输入 andt Tunnel Token (AGK) (例如: eyJhIjo...Ifs9, 若使用Cloudflare Zero Trust隧道请输入, 留空则使用临时隧道): ").strip()
        andt_token = andt_token_input or None # None if empty
    if andt_token:
        print(f"使用 andt Tunnel Token: ******{andt_token[-6:]}") # 仅显示末尾几位
    ...
    elif andt_token: # 如果用了token，必须提供域名
    ...
    config_data = {
        ...
        "andt_token": andt_token, # Will be None if not provided
    ...
    if not andt_token and not custom_domain: # Quick tunnel and no pre-set domain
        print("正在等待临时隧道域名生成...")
        final_domain = get_tunnel_domain()
        if not final_domain:
            print("\033[31m无法获取tunnel域名。请检查andt.log或尝试手动指定域名。\033[0m")
            ...
            sys.exit(1)
    elif andt_token and not custom_domain: # Should have exited earlier, but as a safeguard
...
# 升级脚本
def upgrade():
    script_url = "https://raw.githubusercontent.com/yonggekkk/AndtIS/main/adis_custom_domain.py" # 假设这是最新脚本的地址
...
def check_status():
    ...
    if sb_running and cf_running and LIST_FILE.exists():
        ...
        if CUSTOM_DOMAIN_FILE.exists():
            ...
            elif not config.get("andt_token") and LOG_FILE.exists(): # Quick tunnel, try log
...
def create_startup_script():
    ...
    andt_token = config.get("andt_token") # Safely get token, might be None
...
'''
    ...
    if andt_token: # 使用命名隧道
        cf_cmd = f"{cf_cmd_base} run --token {andt_token}"
    else: # 使用临时隧道
